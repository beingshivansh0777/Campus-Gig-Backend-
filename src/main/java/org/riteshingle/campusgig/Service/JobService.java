...
//    Get all job posted by client
    public Page<JobResponseDTO> getAllJobsPostByMe(String status, Pageable pageable) {
        UserEntity client = authService.getCurrentProfile();
        JobStatus jobStatus;
        ...
        Page<Job> jobs = jobRepository.findJobsByClientIdAndStatus(client.getId(), jobStatus, pageable);
        return jobs.map(this::responseDTO);
    }
...
//    Accept Job Application
    public void acceptJobProposal(Long jobId, Long applicationId) {
        ...
        // Reuses the conversation started at Shortlisted stage (if any),
        // carrying its message history forward; creates one if none exists.
      //  chatService.attachContractToConversation(contract, jobApplication);
    }
